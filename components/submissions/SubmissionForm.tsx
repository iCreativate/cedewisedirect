...
export default function SubmissionForm({ submissionId, initialData }: SubmissionFormProps = {}) {
  ...
  const [forwardTypePickerOpen, setForwardTypePickerOpen] = useState(false);
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      if (e.key === "Escape") {
        setForwardTypePickerOpen(false);
      }
    }
    if (forwardTypePickerOpen) {
      document.addEventListener("keydown", onKeyDown);
      return () => document.removeEventListener("keydown", onKeyDown);
    }
  }, [forwardTypePickerOpen]);
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto w-full max-w-6xl space-y-6 rounded-2xl border border-border/40 bg-card/80 backdrop-blur-md p-8 shadow-xl">
      ...
      {forwardTypePickerOpen && (
        <div
          ...
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setForwardTypePickerOpen(false);
          }}
        >
          <div className="w-full max-w-md overflow-hidden rounded-2xl border border-border/70 bg-background shadow-2xl">
            <div className="border-b bg-gradient-to-r from-muted/30 to-background px-6 py-4">
              <div className="text-base font-semibold text-foreground">Forward to Reinsurer</div>
              <p className="mt-1 text-sm text-muted-foreground">Choose placement type</p>
            </div>
            <div className="grid gap-3 px-6 py-5">
              <button
                type="button"
                className="group rounded-xl border border-border bg-background p-4 text-left hover:bg-muted/40"
                onClick={() => {
                  setForwardTypePickerOpen(false);
                  setForwardType("PROP");
              ...
              >
                ...
                <div className="mt-1 text-xs text-muted-foreground">Proportional</div>
              </button>
              <button
                type="button"
                className="group rounded-xl border border-border bg-background p-4 text-left hover:bg-muted/40"
              ...
              >
                <div className="text-sm font-semibold text-foreground">Non-Prop</div>
                <div className="mt-1 text-xs text-muted-foreground">Non-proportional</div>
              </button>
              <button
                type="button"
                className="rounded-lg border border-border bg-background px-4 py-2 text-sm font-semibold text-foreground hover:bg-muted/50"
                onClick={() => setForwardTypePickerOpen(false)}
              >
                Cancel
              </button>
            </div>
        ...
        {step < 3 ? (
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => setForwardTypePickerOpen(true)}
            >
